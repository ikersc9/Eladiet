...
CLASSIFIER_MODEL = "models/autoencoder_pastillas.pkl"
...
def analyze_single_image():
    """
    Endpoint principal para analizar una única imagen
    ...
    try:
        ...
        # Procesar imagen
        metadata = pill_system.process_images([filepath], output_base_dir=app.config['OUTPUT_FOLDER'])
        
        # Obtener resultados de la primera (y única) imagen
        img_data = metadata['images'][0]
        
        # Leer imagen anotada
        annotated_img = cv2.imread(img_data['annotated_path'])
        _, buffer = cv2.imencode('.jpg', annotated_img)
        img_base64 = base64.b64encode(buffer).decode('utf-8')
        
        # Preparar detecciones para el frontend
        detections = []
        for pill in img_data['pills']:
            detection = {
                'bbox': pill['bbox'],
                'status': 'ok' if pill['classification'] == 'OK' else 'defective',
                'confidence': pill['classification_confidence'],
                'defect_type': map_defect_type(pill['classification'])
            }
        ...
        response = {
            ...
            'stats': {
                'total': img_data['total_pills'],
                'ok': sum(1 for p in img_data['pills'] if p['classification'] == 'OK'),
                'defective': sum(1 for p in img_data['pills'] if p['classification'] == 'NOK')
            }
        }
...
def map_defect_type(classification):
    """Mapea la clasificación a un tipo de defecto específico"""
    if classification == 'OK':
        return None
    elif classification == 'NOK':
        # Aquí podrías añadir lógica más específica si tienes subtipos
        # Por ahora retornamos tipos genéricos
        import random
        types = ['Fracturada', 'Coloración', 'Dimensiones', 'Morfología']
...
def upload_files():
    """Endpoint para subir y procesar múltiples imágenes"""
    if pill_system is None:
        return jsonify({'error': 'Sistema no inicializado'}), 500
